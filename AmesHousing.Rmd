...
library(mice)
library(moments)
train <- read.csv("/Users/lancefernando/Desktop/DataMining/RProjects/IowaHousing/train.csv", header = TRUE)
test <- read.csv("/Users/lancefernando/Desktop/DataMining/RProjects/IowaHousing/test.csv", header = TRUE)
```
...
```{r}
checkNA <- function(tr, te){
  for(i in colnames(tr)){
    percentage <- 0
    
    na.exist <- FALSE
    if(anyNA(tr[,i])){
      print(paste("Train: ", i, "; ", length(which(is.na(tr[,i])))))
      percentage <- percentage + length(which(is.na(tr[,i])))
      na.exist <- TRUE
    }
    if(i %in% colnames(te)){
      if(anyNA(te[,i])){
        print(paste("Test: ", i, "; ", length(which(is.na(te[,i])))))
        percentage <- percentage + length(which(is.na(te[,i])))
        na.exist <- TRUE
      }
    }
    
    if(na.exist){
      print(percentage / (nrow(tr) + nrow(te)))
      print("#######################")
    }
  }
...
Now some numeric variables must be converted into factor form since they must be characterized as categorical. In addition, the creator of the dataset suggests we should drop observations whose **GrLivArea** is greater than 4000. [reference](http://ww2.amstat.org/publications/jse/v19n3/decock.pdf)
...
Now lets perform transformations. We will also be creating new variables that indicate whether or not the value was greater than 0. We will do this because log(0) = -inf.

**skewedWith0** is a vector of variables that are as it says. 

Our new training and testing sets will be log.train and log.test respectively.

```{r}
skewedWith0 <- c("MiscVal",
                 "EnclosedPorch",
                 "OpenPorchSF",
                 "WoodDeckSF",
                 "TotalBsmtSF",
...
#          depth = 10,
#          learn = 0.001,
#          file = "/Users/lancefernando/Desktop/DataMining/RProjects/IowaHousing/Submission40.csv",
#          trainset = log.train,
#          testset = log.test)
